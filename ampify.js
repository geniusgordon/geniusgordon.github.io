...
const ampify = function(html, filePath) {
  ...
  $('head').append(
    $(
      '<link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=Open+Sans%3A400%2C700%2C300%2C800&ver=4.6.1" />'
    )
  );
...
recursive(inputDir, ['amp'], function(err, files) {
  ...
  for (let fileToConvert of filesToConvert) {
    ...
    const newFileLocation = fileToConvert.replace(
      `${inputDir}/`,
      `${inputDir}/${outputDir}/`
    );

    // Create the directory if it doesn't exist
    const newDir = newFileLocation.substring(
      0,
      newFileLocation.lastIndexOf('/')
    );
